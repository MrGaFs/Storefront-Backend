...
const show = async (req: Request, res: Response) => {
	if (isNaN(parseInt(req.params.id)))
		...
		res.json(await usr.show_by_id(parseInt(req.params.id)));
}

const create = async (req: Request, res: Response) => {
	try{
	const userName: string = req.body.user_name, firstName: string = req.body.first_name,
		secondName: string = req.body.second_name, password: string = req.body.password;

	if (userName == undefined || firstName == undefined || secondName == undefined
		|| password == undefined)
		res.status(400).json({ "massage": "the data is incomplete" });
	else
		res.json(await usr.add({
			id: 1, user_name: userName, first_name: firstName,
...
const login = async (req: Request, res: Response) => {
	const userName: string = req.body.user_name, password: string = req.body.password;
...
const usersRoute = (app: express.Application) => {
	...
	app.delete('/users/', jwtAuth, (res: Response, req: Request) => {
		res.status(400).json({ "massage": "Please provide the id" });
	});
	app.put('/users/:id', jwtAuth, update);
	app.put('/users/', jwtAuth, (res: Response, req: Request) => {
		res.status(400).json({ "massage": "Please provide the id" });
	});
}
